...
Keep responses concise and use markdown formatting (lists, bolding) for clarity.`;
	...
	const AIChatModal = ({ open, onClose, chatHistory, isChatLoading, onSendMessage, selectedTest, activeLink, onClearChat }) => {
		const modalClasses = `
						...
						<React.Fragment key={sectionTitle}>
							...
							{links.map((link) => {
								if (link.isCollapsible) {
									...
									return (
										<div key={link.name}>
											<a href="#" onClick={(e) => { e.preventDefault(); handleLinkClick(link.name, true); }} className={`${parentClasses.container}`} style={parentClasses.style}>
											...
											{isSubjectsOpen && link.children && isSidebarOpen && link.children.map(childLink => {
												...
												return (
													<a
														key={childLink.name}
														href="#"
														onClick={(e) => { e.preventDefault(); handleLinkClick(childLink.name, false, true); }}
								...
								return (
									<a key={link.name} href="#" onClick={(e) => { e.preventDefault(); handleLinkClick(link.name); }} className={`${standardLinkClasses.container}`} style={standardLinkClasses.style}>
